...
data2 <- read.csv("result_table_diffGI_data_IFN_endTP.txt",header=T,sep="\t",stringsAsFactors=F);
...
ggplot( data = m ) +
        ...
        geom_text_repel( data=subset( m, fdr < 0.01 & (myDiff > 7.65 | myDiff < -5.5)),aes(x=z.x,y= z.y,label=X),size=3,segment.size=0.3) + 
        labs( title = "Fitm2 - End-point" ) + 
        xlab("CTL_Fitm2 (z, diff GI)") +
        ylab("IFN_Fitm2 (z, diff GI)") +
...
ggsave( "~/Dropbox/CTL_paper/Revised Manuscript/New figures/KRB/scatterplot_Fitm2_CTLvsIFNtx_endPoint.pdf",plot=last_plot(),width=5,height=5,bg="transparent",dpi=300,useDingbats=F);