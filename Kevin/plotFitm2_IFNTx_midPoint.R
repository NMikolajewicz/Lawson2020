...
# Load data
data1 <- read.csv("result_table_diffGI_data_IFN_midTP.txt",header=T,sep="\t",stringsAsFactors=F);
...
ggplot( data = data1 ) +
        ...
        labs( title = "Fitm2 - IFN Tx - Mid-point" ) + 
        xlab("WT (NormZ)") +
        ylab("Fitm2 (NormZ)") +
...
ggsave( "~/Dropbox/CTL_paper/Revised Manuscript/New figures/KRB/scatterplot_Fitm2_IFNtx_midPoint.pdf",plot=last_plot(),width=5,height=5,bg="transparent",dpi=300,useDingbats=F);